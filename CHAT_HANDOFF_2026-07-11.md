 ...
 **Approval model (founder decision 2026-07-21): sign-off happens ONCE, at the PLAN level — the founder approves the whole mission roadmap upfront and never seeds individual missions or tasks again.** Per-mission seeding was an unnecessary gate: if the plan is approved, re-approving each piece of it kills convenience for nothing. The only human gates that remain are the ones that carry NEW risk: SQL approvals, resource/credential gates, budget caps, and any mission NOT in the approved plan.
   **INFRASTRUCTURE AGENCY — the babysitter must own the whole toolchain, not just the loop (founder mandate 2026-07-29, after ~6 hours of manual debugging across Vercel, GitHub, Supabase and the runner in a single session):** every blocker in that session was PLUMBING, not code generation. Catalogued, because this list IS the M4c acceptance criteria — the babysitter must handle each of these WITHOUT the founder:
   ...
   10. **Own diagnosis, not escalation:** when something breaks, the babysitter investigates (read the logs, probe the API, check the SHA) and reports a CAUSE, not a symptom. The founder's job is decisions, never diagnosis.
