...
const {
  ...
  hasSupabaseEnvMock,
  getBusinessByIdMock,
  getExecutionTimelineForBusinessMock,
} = vi.hoisted(() => ({
  requireUserMock: vi.fn(),
  hasSupabaseEnvMock: vi.fn(),
  getBusinessByIdMock: vi.fn(),
  getExecutionTimelineForBusinessMock: vi.fn(),
}));
...
vi.mock("@/lib/execution/status", () => ({
  getExecutionTimelineForBusiness: getExecutionTimelineForBusinessMock,
}));
...
describe("GET /api/businesses/[id]/execution-timeline", () => {
  beforeEach(() => {
    requireUserMock.mockReset();
    hasSupabaseEnvMock.mockReset();
    getBusinessByIdMock.mockReset();
    getExecutionTimelineForBusinessMock.mockReset();
    hasSupabaseEnvMock.mockReturnValue(true);
  });

  it("returns the standard 401 envelope and never queries the business or timeline when unauthenticated", async () => {
    ...
    expect(getExecutionTimelineForBusinessMock).not.toHaveBeenCalled();
  });
  ...
  it("returns the execution timeline when authenticated and owning the business", async () => {
    requireUserMock.mockResolvedValue({ user: { id: "user-1" }, response: null });
    getBusinessByIdMock.mockResolvedValue({ data: { id: "biz-1", user_id: "user-1" }, error: null });
    getExecutionTimelineForBusinessMock.mockResolvedValue({ data: { events: [] }, error: null });

    const response = await GET(new Request("http://localhost"), makeParams("biz-1"));

    expect(response.status).toBe(200);
    await expect(response.json()).resolves.toEqual({ ok: true, data: { events: [] } });
  });
});
