    ...
    <article className="min-w-0 rounded-lg border border-border bg-surface p-3">
      ...
      {capabilities.length > 0 ? (
        <div className="mt-3 flex flex-wrap gap-1.5">
          {capabilities.map((capability) => (
            <span
              key={capability.id}
              className="max-w-full rounded border border-border bg-background px-2 py-1 text-[11px] text-foreground-secondary"
            >
      ...
      {latestRun ? (
        <div className="mt-2 rounded border border-accent/20 bg-accent/8 px-3 py-2.5">
          ...
          {latestRun.summary ? (
            <p
              className="mt-1 break-words text-xs leading-5 text-foreground-secondary"
              title={latestRun.summary}
            >
              {latestRun.summary}
            </p>
          ) : null}
          <p className="mt-2 break-words font-mono text-[11px] uppercase tracking-widest text-muted-foreground">
