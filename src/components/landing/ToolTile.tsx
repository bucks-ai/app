...
export function ToolTile({ tool }: { tool: ToolTileData }) {
  return (
    <GlassPanel interactive className="p-4">
      ...
      <p className="mt-3 text-sm leading-6 text-secondary">{tool.role}</p>
      <p className="mt-4 border-t border-border/70 pt-3 font-mono text-[10px] uppercase tracking-[0.16em] text-muted">
        Signal: <span className="text-secondary">{tool.signal}</span>
      </p>
    </GlassPanel>
  );
}
