...
### 6.3 LEVERAGE DOCTRINE — buy/rent vs. build, and studying prior art (founder concern 2026-07-29: "there are infinitely many AI tools out there for literally everything, and we are barely leveraging any of them")

**The founder's corrective is accepted: the default has been to build from scratch too often, and existing capability is under-leveraged. But "everything can be built" needs a decision rule, because everything built becomes something maintained forever — and maintenance is what kills solo-founder velocity.**

**THE RULE: rent the pipes, build the judgment.**
- **RENT/BUY — commodity capability, where being different has zero value.** Email delivery (Resend), auth/DB (Supabase), payments (Stripe), hosting (Vercel), voice/telephony (Vapi/Retell/Twilio — never build voice infra), deliverability & inbox warmup (specialized, reputational risk if wrong), browser automation infra (Browserbase/Playwright), scraping infra (Apify, official APIs), lead databases (Apollo — **note: an Apollo connector is already available and unused; this is exactly the under-leveraged capability the founder is pointing at**). Nobody buys bucks.ai because it has a better SMTP layer.
- **BUILD — proprietary judgment, where being different IS the product.** The mission engine, the containment model, the market-selection rubric (§6.1), the blueprint compiler, the babysitter, the convenience layer, cross-user learning (M8). These are the moat. They must never be outsourced.
- **STUDY prior art before building either.** Before writing a sequencer, read how Instantly/Clay/Apollo actually structure sequences. Before the market radar, look at how existing pain-mining tools score. This costs an hour and routinely saves a week — and it's the part that's been most neglected. **Taking inspiration is mandatory; reinventing blind is the failure mode.**

**THE ONE REAL TRAP — noted and already agreed by the founder (2026-07-29: the "complete the whole system purely by connecting external AI tools" framing was deliberately rhetorical, to show the *extent* of available leverage, NOT a proposed course of action; founder explicitly ruled it out. Recorded here so no future planner mistakes the illustration for the plan):** if the system is only an orchestration layer over N vendors, then (a) **reliability becomes the product of their reliabilities** — each vendor's rate limits, outages, ToS changes and pricing changes become your downtime, which is a direct threat to the core promise of *running unattended 24/7*; and (b) **there is no moat and no YC story** — "we connected eight APIs" is an integration, not a company. So: vendors may own any *replaceable* piece; the engine that decides, sequences, contains, and self-heals must be ours. Every rented dependency should be swappable behind our own interface, so a vendor dying is a config change, not a rewrite.

**Corollary — never cut corners to avoid asking for a tool (founder mandate 2026-07-29):** if a mission needs a capability we don't have access to, the correct action is to REQUEST the account/tool, never to improvise a worse solution with what's on hand. Workaround-instead-of-asking is an explicit failure mode: it produces silently degraded output that looks finished. Tool acquisition is PRE-APPROVED by the founder — "I have no problem with it." The dependency batch-ahead list (see M4c) should therefore be **generous, not minimal**: list every tool that would make the work materially better, not the bare minimum to limp through.
