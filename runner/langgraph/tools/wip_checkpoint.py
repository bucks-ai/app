...
def checkpoint_wip(
    ...
    **never raises**: this runs on the way out of the process, and an exception
    ...
    # Never run real git from inside the test suite. ``decide_continue_or_stop``
    # is called directly by a dozen existing guard tests, and before this guard
    # existed those tests committed the developer's in-flight work to a
    # ``wip/unknown-task`` branch on every run — the same class of surprise
    # write this module exists to prevent, pointed at the wrong repo. A test
    # that supplies its own ``git_run`` controls every side effect and is
    # allowed through; this mirrors log_tools._maybe_notify_slack's guard
    # against live Slack fan-out under pytest.
    if git_run is None and os.environ.get("PYTEST_CURRENT_TEST"):
        return _blank_result(**{**base, "reason": "suppressed_under_pytest"})
