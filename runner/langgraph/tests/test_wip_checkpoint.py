...
class TestFailureDoesNotBlockExit:
    ...
    def test_real_git_is_suppressed_inside_the_test_suite(self):
        """Regression: a dozen existing guard tests call decide_continue_or_stop
        directly, and this node's checkpoint committed the developer's in-flight
        work to a wip/ branch on every pytest run until this guard existed."""
        assert os.environ.get("PYTEST_CURRENT_TEST"), "guard is keyed on this variable"
        result = checkpoint_wip("/home/arnav/bucks-ai", task_id=None, trigger="guard_stop")

        assert result["checkpointed"] is False
        assert result["reason"] == "suppressed_under_pytest"
