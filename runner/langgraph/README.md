...
| `MISSION_PLANNING` | Run a cheap pre-execution planning pass over all queued tasks after a mission is seeded (M4c.4). Each task gets one short LLM call that predicts expected files, approach, whether the deliverable already exists, and inter-task dependencies. Plans drive conflict detection (shared files → sequential queue ordering + `plan_conflict_detected` event), duplicate detection (`plan_duplicate_detected`), and right-sizing (`plan_oversized_detected`). Plans are also persisted to `.runtime/plans/` as declared scope for M4c.5 enforcement. The pass degrades gracefully — a failed plan call is logged and that task runs as normal (default: true) |
| `MISSION_PLANNING_MAX_FILES` | Number of expected files above which a plan is flagged as oversized and recommended for splitting; logged as `plan_oversized_detected` (default: 10) |
| `MISSION_PLANNING_MODEL` | Anthropic model used for the cheap per-task planning call; intentionally a fast/inexpensive model (default: claude-haiku-4-5-20251001) |
...
| `WIP_CHECKPOINT` | Before the run-loop process exits for any reason (Ctrl+C, SIGTERM, a guard setting a stop reason, an unhandled exception), commit the dirty working tree to the current task's branch under a `WIP checkpoint [...]` message and log `wip_checkpointed` with the sha and file list; the sha is quoted in the stop report so recovery is one `git checkout` away. Never commits to a protected branch — a protected or detached HEAD redirects to `wip/<task-id>` — and never stages a credential-looking file (`.env*`, `*.pem`, `id_rsa`, ...), which is left in the working tree and reported by name. A failed checkpoint is logged loudly (`wip_checkpoint_failed` → Slack) and never blocks the exit (default: true) |
| `WIP_CHECKPOINT_PUSH` | Also push the checkpoint branch to `origin` when a remote exists — a plain `git push -u`, never a force. Set false to keep checkpoints local (default: true) |
...
| `REPO_HEALTH_PREFLIGHT` | Run `scripts/check.sh` once at session start before any task is dispatched (M4c.4). If it fails, the loop stops with `repo_unhealthy` — the broken repo is a pre-existing environment problem, not a task failure, and dispatching workers wastes budget on work that will never pass the post-task verification gate. Also enables per-task base-check classification: when a worker's check fails mid-task, a second check.sh run on the stashed clean state determines whether the failure is environmental (no counters charged) or task-caused (existing retry handling). Disable only for environments where `scripts/check.sh` cannot run (default: true) |
| `REPO_HEALTH_PREFLIGHT_TIMEOUT_S` | Seconds before the repo-health `scripts/check.sh` subprocess is hard-killed; prevents a hung npm/tsc process from wedging startup indefinitely. `scripts/check.sh` was observed at ~90s; 120s gives 33% headroom (default: 120) |
| `NETWORK_PAUSE_ENABLED` | Treat network-layer failures as a temporary pause rather than task failures. When enabled, a connectivity probe (DNS + HEAD) runs before any task is claimed and before the repo-health preflight; if the machine is offline the loop exits with `network_unavailable` and the watchdog retries every `NETWORK_RETRY_DELAY_S` seconds. Disabling restores pre-M4c.4 behaviour where a drive between locations burns transient-retry counters (default: true) |
| `NETWORK_PROBE_TIMEOUT_S` | Seconds before the connectivity probe's DNS resolution and HEAD request time out; keep low so an offline startup costs nothing (default: 5) |
| `NETWORK_RETRY_DELAY_S` | Seconds the watchdog waits between restarts when the stop reason is `network_unavailable`; distinct from `WATCHDOG_RESTART_DELAY_S` because each restart re-enters the startup flow (hook install, repo-health check.sh) at ~90s of CPU — 30s restarts while offline is wasteful (default: 300) |
| `NETWORK_MAX_PATIENCE_MINUTES` | Minutes of consecutive offline time before the watchdog stops restarting and reports that the machine has had no internet for N minutes; prevents retrying a dead network forever (default: 90) |
...
| `STRIPE_SECRET_KEY` | Stripe restricted secret key — creates Connect Express sub-accounts for customer payment flows via `tools/provisioning.py`; generate at dashboard.stripe.com → Developers → API Keys with `write` on Connect accounts (required for Stripe child provisioning) |
| `RESEND_API_KEY` | Resend parent API key — creates sending domains and scoped child API keys via `tools/provisioning.py`; generate at resend.com → API Keys (required for Resend child provisioning) |
| `TWILIO_ACCOUNT_SID` | Twilio master account SID — creates subaccounts via `tools/provisioning.py`; find at console.twilio.com → Account → General Settings (required for Twilio child provisioning) |
| `TWILIO_AUTH_TOKEN` | Twilio master auth token — paired with `TWILIO_ACCOUNT_SID` for subaccount creation; find at console.twilio.com → Account → General Settings (required alongside `TWILIO_ACCOUNT_SID`) |
| `POSTHOG_ORG_ID` | PostHog organisation slug for child project creation via `tools/provisioning.py`; find at posthog.com → your org → Settings → General (required for PostHog child provisioning) |
