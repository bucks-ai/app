...
| `WIP_CHECKPOINT` | Before the run-loop process exits for any reason (Ctrl+C, SIGTERM, a guard setting a stop reason, an unhandled exception), commit the dirty working tree to the current task's branch under a `WIP checkpoint [...]` message and log `wip_checkpointed` with the sha and file list; the sha is quoted in the stop report so recovery is one `git checkout` away. Never commits to a protected branch — a protected or detached HEAD redirects to `wip/<task-id>`. A failed checkpoint is logged loudly (`wip_checkpoint_failed` → Slack) and never blocks the exit (default: true) |
